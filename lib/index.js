...
async function computeStats(ctx, folds, titles, caches) {
  ...
  // Cheap per-log change tokens (headers only, no full-log parse): lets the
  // incremental fold skip re-reading logs that did not change since last poll.
  let revById = new Map()
  try {
    const snaps = await persistence.listSnapshots()
    for (const s of snaps) revById.set(s.header.id, s.revision)
  } catch (e) { revById = new Map() }
  ...
  await pool(coveredList, 6, async (rec) => {
    ...
    try {
      if (isLive) {
        // Incremental: read only events after our watermark, and only when the
        // persisted log's revision changed since the last fold. readFrom bounds
        // what is returned/refolded (no full-log clone per poll). A live session
        // that is not yet persisted falls back to one full live read.
        const rev = revById.has(id) ? revById.get(id) : null
        if (fold.rev === null || fold.rev !== rev) {
          let folded = false
          try {
            const tail = await persistence.readFrom(id, fold.lastSeq + 1)
            ...
            if (rev !== null) fold.rev = rev
            folded = true
          } catch (e) { /* fall through to the full live read below */ }
          if (!folded) {
            try {
              ...
              if (!fold.errorMsg) fold.errorMsg = ((e2 && e2.message) || String(e2))
            }
          }
        }
        try {
