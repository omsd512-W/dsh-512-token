...
async function computeStats(ctx, folds, titles, caches) {
  ...
  await pool(coveredList, 6, async (rec) => {
    ...
    try {
      ...
      try {
        // ponytail: live observations materialize full logs; switch to handle.read tails if 200 large live sessions make polling slow.
...
function apply(ctx) {
  ...
  ctx.inject(['webServer'], (httpCtx) => {
    ...
    }), 'dsh-512-token: /token-stats route')
  })
}
