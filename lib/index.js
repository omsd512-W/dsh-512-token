...
async function loadFold(sessionQuery, rec, folds, titles, signal) {
  ...
  try {
    ...
    if (fold.coldValues) {
      ...
      const observation = await sessionQuery.observeSession(id, { signal })
      try {
  ...
  if (title === undefined) {
    try {
      const t = await sessionQuery.readTitle(id, signal)
      title = (t && t.title) || ''
    } catch (e) { title = '' }
    titles.set(id, title)
  }
  return { fold, usage, stats, pressure, title }
}
...
async function computeStats(ctx, folds, titles, caches) {
  ...
  const targetList = records.slice(0, 200)
  const missing = targetList.filter((rec) => !rec.live && !caches.ready.has(rec.header.id) && !caches.loading.has(rec.header.id))
  for (const rec of missing) caches.loading.add(rec.header.id)
  if (missing.length) {
    void pool(missing, 6, async (rec) => {
      if (caches.abort.signal.aborted) return
      await loadFold(sessionQuery, rec, folds, titles, caches.abort.signal)
      if (!caches.abort.signal.aborted) caches.ready.add(rec.header.id)
    }).catch((e) => { ctx.logger.warn('dsh-512-token: history warmup failed: ' + ((e && e.message) || String(e))) })
      .finally(() => { for (const rec of missing) caches.loading.delete(rec.header.id) })
  }
  const coveredList = targetList.filter((rec) => rec.live || caches.ready.has(rec.header.id))
  ...
  await pool(coveredList, 6, async (rec) => {
    const id = rec.header.id
    const isLive = rec.live
    const { fold, usage, stats, pressure, title } = await loadFold(sessionQuery, rec, folds, titles)
    caches.ready.add(id)
  ...
  return {
    ...
    warming: targetList.length - coveredList.length,
    totalCount,
  }
}
...
function apply(ctx) {
  ...
  const caches = { providers: { ts: 0, names: new Map(), configured: new Set() }, ready: new Set(), loading: new Set(), abort: new AbortController() }
  ctx.effect(() => () => caches.abort.abort(), 'dsh-512-token: stop history warmup')
